...
if ($_POST) { //Si le formulaire est envoyé
    #   Validation du formulaire :
        if (!isset($_POST['pseudo']) || strlen($_POST['pseudo']) < 4 || strlen($_POST['pseudo']) > 20 ) 
            ...
            $contenu .= '<div class="bg-danger"> La ville doit contenir entre 2 et 20 caractères. </div>';

            if (!isset($_POST['adresse']) || strlen($_POST['adresse']) < 4 || strlen($_POST['adresse']) > 20 ) 
            $contenu .= '<div class="bg-danger"> L\'adresse doit contenir entre 4 et 20 caractères. </div>';

            if (!isset($_POST['civilite']) || ($_POST['civilite'] != 'm' && $_POST['civilite'] != 'f' )) 
                 $contenu .= '<div class="bg-danger"> La civilite est incorrecte. </div>';
                
            if (!isset($_POST['code_postal']) || !preg_match('#^[0-9]{5}$#',$_POST['code_postal'])  ) 
                 $contenu .= '<div class="bg-danger"> Le code postale est incorrect. </div>'; //l'expression rationnelle (ou régulière) est encadrée par des #. Le ^ signifie que le code postal "commence par" et le $ signifie "finit par".
                 #Lz présence des deux symboles en même temps signifie qu'on définit l'intégralité de l'expression. [0-9] définit les caractères autorisé de 0 à 9. {5} définit strictement le nombre de ces caractères. La fonction prédéfinie preg_match retourne 0 si le code postal n'estpas correct, sinon 1.
            ...
            //Si plus d'erreur dur le formulaire, on vérifie la disponibilité du pseudo avec d'inscrire le membre en BDD :
...
if ($affiche_formulaire) : // si internaute n'est pas incrit, cette variable valant true, on entre dans la condition et on affiche le formulaire
?>
...
<form action="" method="post">
    ...
    <input type="nom" id="nom" name="nom" value="<?php echo $_POST['nom'] ?? '';    ?>"> <br><br>

    <label for="prenom">Prénom</label> <br>
    <input type="prenom" id="prenom" name="prenom" value="<?php echo $_POST['prenom'] ?? '';    ?>"> <br><br>
    ...
    <input type="radio" name="civilite" value="f" <?php if (isset($_POST['civilite']) && $_POST['civilite'] == 'f') echo 'checked'; {
    }  ?> > Femme <br> <br>


     <label for="ville">Ville</label> <br>
    <input type="text" name="ville" id="ville" value="<?php echo $_POST['ville'] ?? '';    ?>"> <br> <br>

    
     <label for="code_postal">Code postal</label> <br>
    <input type="text" name="code_postal" id="code_postal" value=" <?php echo $_POST['code_postal'] ?? '';    ?>"> <br> <br>

 <label for="adresse">Adresse</label> <br>
    <textarea name="adresse" id="adresse" cols="30" rows="10"><?php echo $_POST['adresse'] ?? '';    ?></textarea> <br> <br>

    <input type="submit" value="S'inscrire" name="inscriptions" class="btn">
